...
struct SegTree2D {
    ...
    long long merge(long long a, long long b) {
        return a + b;  // switch to max(a, b) for a rectangle-max tree
    }

    void update(int r, int c, long long val) {
        int row_leaf = r + rows, col_leaf = c + cols;
        tree[row_leaf][col_leaf] = val;  // use += val for an add-to-cell variant
    ...
    long long query_cols(int row_node, int c1, int c2) {
        ...
        for (int l = c1 + cols, r = c2 + cols + 1; l < r; l >>= 1, r >>= 1) {
            if (l & 1) res = merge(res, tree[row_node][l++]);
            if (r & 1) res = merge(res, tree[row_node][--r]);
        }
        return res;
    }
    ...
    long long query(int r1, int c1, int r2, int c2) {
        ...
        for (int l = r1 + rows, r = r2 + rows + 1; l < r; l >>= 1, r >>= 1) {
            if (l & 1) res = merge(res, query_cols(l++, c1, c2));
            if (r & 1) res = merge(res, query_cols(--r, c1, c2));
        }
        return res;
    }
};
...
/**
 * Example: point-update a 3x4 grid, then sum a rectangle, a single cell, and
 * the whole grid.
 */
int main() {
    SegTree2D st(3, 4);
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 4; j++) st.update(i, j, grid[i][j]);
