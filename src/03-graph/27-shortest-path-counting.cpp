...
/**
 * Number of distinct shortest paths from src to every node, strictly positive
 ...
 *
 * Contract: edge weights must be positive. Dijkstra still finds distances with
 * zero-weight edges, but counting then requires separately processing the
 * equal-distance edges (and defining how zero-weight cycles count). ways[] is
 * the path count taken modulo MOD; ways[v] == 0 means v is unreachable. Parallel
 * edges and multiple equal-length routes are counted as distinct paths.
 */
...
/** Example: two equal-length shortest routes from 0 to 3. */
int main() {
    n = 4;
    adj.assign(n, {});
