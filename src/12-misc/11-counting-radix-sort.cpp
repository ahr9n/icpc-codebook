 ...
 *
 * counting_sort(a, max_val): stable, O(n + max_val). Requires max_val >= 0 and
 *   0 <= a[i] <= max_val.
...
void radix_sort(vector<int>& a) {
    ...
    for (int pass = 0; pass < byte_count; pass++) {
        ...
        for (int bucket = 1; bucket <= radix; bucket++) {
    ...
    for (int i = 0; i < n; i++) {
        a[i] = (int)(key[i] ^ sign_bit);
    }
}

/** Example: counting sort small keys and radix sort signed 32-bit keys. */
int main() {
    vector<int> a = {4, 1, 3, 4, 0, 2, 1};
    vector<int> sorted_small = counting_sort(a, 4);
    for (int x: sorted_small) {
        cout << x << " ";
    }
    cout << "\n";  // -> 0 1 1 2 3 4 4
